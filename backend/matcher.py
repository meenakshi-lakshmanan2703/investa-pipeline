...
def normalize(val: Optional[str]) -> str:
    if not val:
        return ""
    # Strip common suffixes so "Bernau bei Berlin" matches "Bernau"
    val = val.lower().strip()
    for suffix in [" bei berlin", " bei hamburg", " am rhein", " an der "]:
        val = val.split(suffix)[0]
    return val

def check_for_duplicate(incoming_street: Optional[str], incoming_city: str, threshold: float = 80) -> Tuple[bool, Optional[str]]:
    ...
    for record in get_known_properties():
        ...
        if city_score >= 75:  # fuzzy city match too
